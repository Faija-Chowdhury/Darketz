            ...
            <div class="col-sm-12 col-md-7 pl-md-5 mt-5 mt-md-0 mr-auto">
              <div class="product-section-information">
                ...
                <div class="my-2">
                  <h4>Price:</h4>
                  @if($product->onSale)
                  <p class="text-orange h4">Tk.{{number_format($product->sale_price)}}</p>
                  <p>
                    <span class="line-through h5"> Tk.{{$product->price}}</span> 
                    <span class="btn btn-primary disabled ml-3"> {{ floor((($product->price - $product->sale_price)/$product->price)*100) }}% off</span> 
                  </p>
                  @else
                  <p class="text-orange h4">Tk.{{number_format($product->price)}}</p>
                  @endif
                </div>
  ...
  function cartIncrement(){
    ...
    if(Number($('#cartCount').val()) < Number($('#stockVal').val()) ){
      if(Number($('#cartCount').val() < 5)){
        $('#cartCount').val(paTkeInt($('#cartCount').val()) + 1)
      }
    }else{
